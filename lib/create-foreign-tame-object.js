...
module.exports = (membrane, release, capture) => {
  ...
  handlers.apply = (target, value, values) => {
    ...
    return membrane.enter(capture(Reflect_apply(object, release(value), values)));
  };
  ...
  handlers.construct = (target, values) => {
    ...
    for (let index = 0, length = values.length; index < length; index++)
      values[index] = release(membrane.leave(values[index]));
    return membrane.enter(capture(Reflect_construct(object, values)));
  };
    ...
    if (descriptor2) {
      if ("value" in descriptor2) {
        if (descriptor2.writable || descriptor2.configurable)
          return membrane.enter(capture(descriptor2.value));
        descriptor2.value = membrane.enter(capture(descriptor2.value));
        Reflect_defineProperty(target, key, descriptor2);
        return descriptor2.value;
      }
      if ("get" in descriptor2)
        return membrane.enter(capture(Reflect_apply(descriptor2.get, release(receiver), [])));
      return void 0;
    }
    const prototype = Reflect_getPrototypeOf(object);
    if (prototype)
      return Reflect_get(capture(prototype), key, receiver);
    if (key === "__proto__")
  ...
  handlers.getOwnPropertyDescriptor = (target, key) => {
    ...
    if (descriptor2) {
      if ("value" in descriptor2) {
        ...
        descriptor2.get = capture(descriptor2.get);
        descriptor2.set = capture(descriptor2.set);
      }
  ...
  handlers.getPrototypeOf = (target) => capture(Reflect_getPrototypeOf(objects.get(target)));
  ...
  handlers.isExtensible = (target) => {
    ...
    if (Reflect_isExtensible(target)) {
      ...
      Reflect_setPrototypeOf(target, capture(Reflect.getPrototypeOf(object)));
  ...
  handlers.set = (target, key, value, receiver) => {
    ...
    if (descriptor) {
      if (descriptor.writable) {
        if (release(receiver) === object) {
          descriptor.value = release(membrane.leave(value));
      ...
      if ("set" in descriptor) {
        Reflect_apply(descriptor.set, release(receiver), [release(membrane.leave(value))]);
        return true;
      }
      return false;
    }
    const prototype = Reflect_getPrototypeOf(object);
    if (prototype)
      return Reflect_set(capture(prototype), key, value, receiver);
    if (key === "__proto__")
      return false;
  ...
  handlers.setPrototypeOf = (target, prototype) => Reflect_setPrototypeOf(objects.get(target), release(prototype));

  return (object) => {
    const target = (
      Array_isArray(object) ?
      [] :
      (
        typeof object === "function" ?
        (Reflect.getOwnPropertyDescriptor(object, "prototype") ? (function () {}) : (() => {})) :
        {}));
