...
module.exports = (membrane, capture, release) => {
  ...
  handlers.apply = (target, value, values) => {
    ...
    return release(membrane.leave(Reflect_apply(object, capture(value), values)));
  };
  ...
  handlers.construct = (target, values) => {
    ...
    for (let index = 0, length = values.length; index < length; index++)
      values[index] = membrane.enter(capture(values[index]));
    return release(membrane.leave(Reflect_construct(object, values)));
  };
    ...
    if (descriptor2) {
      if ("value" in descriptor2) {
        if (descriptor2.writable || descriptor2.configurable)
          return release(membrane.leave(descriptor2.value));
        descriptor2.value = release(membrane.leave(descriptor2.value));
        Reflect_defineProperty(target, key, descriptor2);
        return descriptor2.value;
      }
      if ("get" in descriptor2)
        return release(membrane.leave(Reflect_apply(descriptor2.get, capture(receiver), [])));
      return void 0;
    }
  ...
  handlers.getOwnPropertyDescriptor = (target, key) => {
    ...
    if (descriptor2) {
      if ("value" in descriptor2) {
        ...
        descriptor2.get = release(descriptor2.get);
        descriptor2.set = release(descriptor2.set);
      }
  ...
  handlers.getPrototypeOf = (target) => release(Reflect_getPrototypeOf(objects.get(target)));
  ...
  handlers.isExtensible = (target) => {
    ...
    if (Reflect_isExtensible(target)) {
      ...
      Reflect_setPrototypeOf(target, release(Reflect.getPrototypeOf(object)));
  ...
  handlers.set = (target, key, value, receiver) => {
    ...
    if (descriptor) {
      if (descriptor.writable) {
        if (capture(receiver) === object) {
          descriptor.value = membrane.enter(capture(value));
      ...
      if ("set" in descriptor) {
        Reflect_apply(descriptor.set, capture(receiver), [membrane.enter(capture(value))]);
        return true;
      }
      return false;
    }
  ...
  handlers.setPrototypeOf = (target, prototype) => Reflect_setPrototypeOf(objects.get(target), capture(prototype));
